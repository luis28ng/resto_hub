...
const OrderForm = ({ customer }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (reservationCode) {
                fetchCustomerOrders(reservationCode)
            }
        };
    ...
    const handleAddOrder = async (orderRequest) => {
        console.log('My order request', orderRequest)
        try {
    ...
    const fetchCustomerOrders = async (reservationCode) => {
        try {
    ...
    return (
        <div>
            {foodOrders.length !== 0 ? (
                <OrderTable foodOrders={foodOrders}
